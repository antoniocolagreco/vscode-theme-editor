...
describe("extractColorsFromTheme", () => {
  it("should extract colors from UI colors", () => {
    const theme: Partial<VSCodeTheme> = {
      colors: new Map([
        [
          "editor.background",
          { colorStyle: { name: "Dark BG", value: "#1e1e1e", scopes: new Set() } },
        ],
        [
          "editor.foreground",
          { colorStyle: { name: "Light Text", value: "#d4d4d4", scopes: new Set() } },
        ],
      ]),
    }
  ...
  it("should extract colors from semantic token colors", () => {
    const theme: Partial<VSCodeTheme> = {
      semanticTokenColors: new Map([
        [
          "variable",
          { foreground: { name: "Variable Blue", value: "#9CDCFE", scopes: new Set() } },
        ],
        [
          "function",
          { foreground: { name: "Function Yellow", value: "#DCDCAA", scopes: new Set() } },
        ],
      ]),
    }
  ...
  it("should not create duplicates for same color value", () => {
    const theme: Partial<VSCodeTheme> = {
      colors: new Map([
        [
          "editor.background",
          { colorStyle: { name: "Dark", value: "#1e1e1e", scopes: new Set() } },
        ],
        [
          "sidebar.background",
          { colorStyle: { name: "Dark", value: "#1e1e1e", scopes: new Set() } },
  ...
  it("should extract colors from all sections combined", () => {
    const theme: Partial<VSCodeTheme> = {
      colors: new Map([
        ["editor.background", { colorStyle: { name: "BG", value: "#1e1e1e", scopes: new Set() } }],
      ]),
      tokenColors: new Map([
        ["comment", { foreground: { name: "Comment", value: "#6A9955", scopes: new Set() } }],
      ]),
      semanticTokenColors: new Map([
