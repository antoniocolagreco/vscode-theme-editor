      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {isAddingNew
                ? "Create a new semantic token color scope"
                : "Modify the semantic token color scope"}
            </DialogDescription>
          </DialogHeader>
...
export function SemanticTokensPage() {
  ...
  const semanticTokens = useMemo(
    () => Array.from(theme?.semanticTokenColors?.entries() || []),
  ...
  const validateSaveInput = useCallback(
    ...
    ): { valid: boolean; foreground?: ColorStyle; fontStyle?: FontStyle } => {
      if (!scopeName.trim()) {
  ...
  const handleEdit = useCallback(
    (scope: string, currentSemanticToken: SemanticTokenColor) => {
  ...
  const handleDelete = useCallback(
    (scope: string, semanticToken: SemanticTokenColor) => {
      if (!theme) {
        return
      }
  ...
  const buildSemanticToken = useCallback(
    (foreground: ColorStyle | undefined, fontStyle: FontStyle | undefined): SemanticTokenColor => {
  ...
  const handleDialogSave = useCallback(
    (scopeName: string, foregroundName: string, fontStyleValue: string) => {
      if (!theme) {
        return
      }
      ...
      const oldSemanticToken = editingScope
        ? theme.semanticTokenColors?.get(editingScope)
        : undefined
    ...
    <>
      <Card className='flex flex-col h-full'>
        <CardHeader>
          <CardTitle>Semantic Tokens</CardTitle>
          <CardDescription>
            Manage VS Code semantic token color scopes. Semantic tokens provide semantic
            highlighting based on language understanding rather than textual patterns.
          </CardDescription>
        </CardHeader>
          ...
          <ScrollArea className='flex-1'>
            {filteredSemanticTokens.length === 0 ? (
              <div className='flex flex-col items-center justify-center py-12 text-center'>
                ...
                <p className='text-sm text-muted-foreground'>
                  {search
                    ? "No semantic token scopes match your search"
