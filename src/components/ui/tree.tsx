...
const useTreeNode = () => {
  ...
  if (!context) {
    throw new Error("TreeNode components must be used within a")
  }
  return context
}
  ...
  return (
    <motion.div
      className={cn(
        ...
        isSelected && "bg-slate-100/80 dark:bg-slate-800/80",
        className
      )}
      onClick={e => {
  ...
  return (
    <div className='pointer-events-none absolute top-0 bottom-0 left-0'>
      ...
      {Array.from({ length: level }, (_, index) => {
        ...
        return (
          <div
            className='absolute top-0 bottom-0 border-slate-200/40 border-l dark:border-slate-800/40'
            key={index.toString()}
            style={{
      ...
      <div
        className='absolute top-1/2 border-slate-200/40 border-t dark:border-slate-800/40'
        style={{
      ...
      {isLast && (
        <div
          className='absolute top-0 border-slate-200/40 border-l dark:border-slate-800/40'
          style={{
            left: (level - 1) * (indent ?? 0) + 12,
            height: "50%",
          }}
...
export const TreeIcon = ({ icon, hasChildren = false, className, ...props }: TreeIconProps) => {
  ...
  return (
    <motion.div
      className={cn(
        "mr-2 flex h-4 w-4 items-center justify-center text-slate-500 dark:text-slate-400",
        className
      )}
